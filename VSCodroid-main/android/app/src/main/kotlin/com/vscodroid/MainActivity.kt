...
class MainActivity : AppCompatActivity() {
    ...
    private fun injectBridgeToken() {
        ...
        // Polyfill long press for right-click
        injectLongPressRightClickPolyfill()
        // Polyfill pinch to zoom to map to VS Code Zoom
        injectPinchToZoomPolyfill()
    ...
    /**
     * Injects JavaScript to add long-press to right-click (contextmenu) behavior.
     */
    private fun injectLongPressRightClickPolyfill() {
        webView?.evaluateJavascript(
            """
            (function() {
                if (window.__vscodroidLongPressActive) return;
                window.__vscodroidLongPressActive = true;
                var touchTimer;
    ...
    /**
     * Injects JavaScript to add pinch-to-zoom support without breaking desktop layout.
     * Maps two finger touch scaling to VS Code's workbench zoom commands.
     */
    private fun injectPinchToZoomPolyfill() {
        webView?.evaluateJavascript(
            """
            (function() {
                if (window.__vscodroidPinchZoomActive) return;
                window.__vscodroidPinchZoomActive = true;
                ...
                window.addEventListener('touchstart', function(e) {
                    if (e.touches.length === 2 && e.cancelable) {
                        e.preventDefault(); // Prevent native zoom if it wasn't disabled
                ...
                window.addEventListener('touchmove', function(e) {
                    if (e.touches.length === 2 && e.cancelable) {
                        ...
                        var delta = curDist - lastDist;
                        lastDist = curDist;
                        
                        zoomAccumulator += delta;
                        
                        // Execute zoom via synthesized keyboard shortcuts (Ctrl+= and Ctrl+-)
                        var target = document.activeElement || document.body;
                        if (zoomAccumulator > 150) {
                            var initIn = { key: '+', code: 'Equal', keyCode: 187, which: 187, ctrlKey: true, bubbles: true, cancelable: true, composed: true };
                            target.dispatchEvent(new KeyboardEvent('keydown', initIn));
                            target.dispatchEvent(new KeyboardEvent('keyup', initIn));
                            zoomAccumulator = 0;
                        } else if (zoomAccumulator < -150) {
                            var initOut = { key: '-', code: 'Minus', keyCode: 189, which: 189, ctrlKey: true, bubbles: true, cancelable: true, composed: true };
                            target.dispatchEvent(new KeyboardEvent('keydown', initOut));
                            target.dispatchEvent(new KeyboardEvent('keyup', initOut));
                            zoomAccumulator = 0;
                        }
                    }
                }, { passive: false });
